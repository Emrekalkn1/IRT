let mevcutProjeId = null, mevcutProjeKod = null, mevcutProjeAd = '', mevcutProjeTestTuru = 'standart', mevcutProjeCevapSayisi = 0;
    let aiRaporProjeId = null;
    let aiRaporSlidePack = null;
    let seciliTakipOturumId = null, seciliTakipIsim = null;
    let hamVeriSayfa = 1, katilimciAnalizSayfa = 1;
    const tabloSayfaBoyutu = 10;
    let chartExplicit, chartImplicit, chartGap, chartRadar, chartScatter;
    let mevcutAgGorunumu = 'ag';
    ...
    function isMcrtTest(value) {
        return value === 'mcrt' || value === 'mrt';
    }

    function renderPagerHTML(prefix, paging, totalLabel) {
        const sayfa = Number(paging?.sayfa || 1);
        const toplamSayfa = Number(paging?.toplam_sayfa || 1);
        const toplamKayit = Number(paging?.toplam_kayit || 0);
        const oncekiDisabled = sayfa <= 1 ? 'disabled' : '';
        const sonrakiDisabled = sayfa >= toplamSayfa ? 'disabled' : '';
        return `
            <div class="table-pager-info">${escapeHTML(totalLabel)}: <strong>${toplamKayit}</strong> • Sayfa <strong>${sayfa}</strong> / <strong>${toplamSayfa}</strong></div>
            <div class="table-pager-actions">
                <button class="btn btn-outline btn-sm" data-action="${prefix}-prev" ${oncekiDisabled}>Önceki</button>
                <button class="btn btn-outline btn-sm" data-action="${prefix}-next" ${sonrakiDisabled}>Sonraki</button>
            </div>
        `;
    }

    function adminArayuzBagla() {
        const clickBagla = (id, handler) => {
            const el = document.getElementById(id);
            if (el) el.addEventListener('click', handler);
        };
        const changeBagla = (id, handler) => {
            const el = document.getElementById(id);
            if (el) el.addEventListener('change', handler);
        };

        clickBagla('btnTemaDegistir', temaDegistir);
        clickBagla('btnProjeOlustur', projeOlustur);
        clickBagla('btnProjeYedekYukle', projeYedekSeciciAc);
        clickBagla('btnFiltreTemizle', filtreTemizle);
        clickBagla('btnAiPdf', aiRaporPdfIndir);
        ...
        clickBagla('btnIfadeEkle', ifadeEkle);
        if (typeof mcrtSecenekEkle === 'function') {
        ...
        changeBagla('inputProjeYedekDosya', projeYedekYukle);
        ['fCinsiyet', 'fSes', 'fYasMin', 'fYasMax'].forEach(id => changeBagla(id, analizYukle));
        changeBagla('analysisViewFilter', analizGorunumuDegisti);
        changeBagla('analysisAggregationFilter', analizSeviyesiDegisti);
        changeBagla('scatterIfadeFiltre', scatterGuncelleLocal);
        changeBagla('networkViewFilter', agGorunumuDegisti);
        ['editHedefOrneklem', 'editSoruRandomize', 'editPanelComplete', 'editPanelScreenout', 'editPanelQuotaFull', 'editMcrtYerlesim'].forEach(id => changeBagla(id, projeGuncelle));
        changeBagla('editProjeDurum', projeDurumGuncelle);
        changeBagla('editTestTuru', () => { mcrtArayuzGuncelle(); projeGuncelle(); });
        changeBagla('editMcrtKurgu', () => { mcrtArayuzGuncelle(); projeGuncelle(); });

        document.addEventListener('click', (event) => {
            const tabEl = event.target.closest('[data-tab-target]');
            if (tabEl) {
                tabDegistir(tabEl.dataset.tabTarget);
                return;
            }
            ...
            switch (action) {
                case 'toggle-fullscreen':
                    toggleCardFullscreen(actionEl);
                    break;
                case 'dismiss-ai-oneri':
                    const kutu = actionEl.closest('#aiOneriKutusu');
                    if (kutu) kutu.style.display = 'none';
                    break;
                case 'proje-detay':
                    projeDetayAc(actionEl.dataset.projeId);
                    break;
                case 'proje-sil-hizli':
                    event.preventDefault();
                    event.stopPropagation();
                    projeSilHizli(actionEl.dataset.projeId, actionEl.dataset.projeAd || '');
                    break;
                case 'marka-sil':
                    markaSil(actionEl.dataset.markaId);
                    break;
                case 'ifade-sil':
                    ifadeSil(actionEl.dataset.ifadeId);
                    break;
                case 'kategori-sec':
                    kategoriSec(actionEl.dataset.kategori || '');
                    break;
                case 'kategori-hizli-ekle':
                    kategoriIcinHizliIfadeEkle(actionEl.dataset.kategori || '', actionEl.dataset.metin || '');
                    break;
                case 'kategori-yenile':
                    aiKategoriIfadeleriAl(actionEl.dataset.kategori || '', true);
                    break;
                case 'katilimci-sec':
                    katilimciSec(actionEl.dataset.oturumId || '', actionEl.dataset.pid || '', actionEl.dataset.isim || '', actionEl);
                    break;
                case 'katilimci-sec-kaldir':
                    katilimciSecKaldir();
                    break;
                case 'ham-veri-prev':
                    hamVeriSayfa = Math.max(1, hamVeriSayfa - 1);
                    renderHamVeri();
                    break;
                case 'ham-veri-next':
                    hamVeriSayfa += 1;
                    ...
                    break;
                case 'katilimci-prev':
                    katilimciAnalizSayfa = Math.max(1, katilimciAnalizSayfa - 1);
                    renderKatilimciAnalizi();
                    break;
                case 'katilimci-next':
                    katilimciAnalizSayfa += 1;
                    renderKatilimciAnalizi();
                    break;
                case 'link-yeniden-ac':
                    linkYenidenAc(actionEl.dataset.token || '');
                    break;
            }
        });
    }
    ...
    async function projeleriYukle() {
        ...
        document.getElementById('projeGrid').innerHTML = data.projeler.map(p => {
            ...
            return `
                <div class="card" data-action="proje-detay" data-proje-id="${p.id}" style="cursor:pointer; position:relative; overflow:hidden;">
                    <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:1rem;">
                        ...
                        <div style="display:flex; flex-direction:column; align-items:flex-end; gap:0.4rem;">
                            ...
                            <button class="btn btn-danger btn-sm" style="padding:0.2rem 0.4rem; font-size:0.6rem;" data-action="proje-sil-hizli" data-proje-id="${p.id}" data-proje-ad="${escapeHTML(p.ad)}">SİL</button>
                        </div>
                    </div>
    ...
    async function projeDetayAc(id) {
        ...
        const aiPdfBtn = document.getElementById('btnAiPdf');
        if (aiPdfBtn) aiPdfBtn.style.display = 'none';
        aiRaporProjeId = null;
        ...
        const agSecici = document.getElementById('networkViewFilter');
        if (agSecici) {
            agSecici.value = isMcrtTest(mevcutProjeTestTuru) ? 'isi' : 'ag';
            mevcutAgGorunumu = agSecici.value;
        }
        ...
        document.getElementById('markaListesi').innerHTML = p.markalar.map(m => {
            ...
            <li class="item-row">
                ...
                <button class="btn btn-danger btn-sm" data-action="marka-sil" data-marka-id="${m.id}">✕</button>
            </li>`;
        }).join('');
    ...
    function projeYedekSeciciAc() {
        const input = document.getElementById('inputProjeYedekDosya');
        if (!input) return;
        input.value = '';
        input.click();
    }

    async function projeYedekYukle(event) {
        const input = event?.target;
        const file = input?.files?.[0];
        if (!file) return;
        if (!confirm(`'${file.name}' yedeği yeni bir proje olarak geri yüklensin mi? Mevcut projeler korunur.`)) {
            input.value = '';
            return;
        }
        ...
        try {
            toastGoster('Yedek yukleniyor, lutfen bekleyin...');
            const res = await fetch('/api/proje/yedek_yukle', {
                method: 'POST',
            ...
            const data = await res.json();
            if (data.durum !== 'basarili') {
                throw new Error(data.mesaj || 'Yedek yuklenemedi.');
            }
    ...
    async function aiKategoriOnerisiAl() {
        ...
        try {
            ...
            liste.innerHTML = kategoriler.map(k => `
                <button class="btn btn-outline btn-sm" style="background:white;" data-action="kategori-sec" data-kategori="${escapeHTML(k)}">${escapeHTML(k)}</button>
            `).join('');
                ...
                <div style="border:1px solid var(--border); border-radius:12px; padding:0.85rem; background:rgba(255,255,255,0.02);">
                    <div style="display:flex; justify-content:space-between; align-items:center; gap:0.75rem; margin-bottom:0.65rem; flex-wrap:wrap;">
                        <div style="display:flex; align-items:center; gap:0.5rem; flex-wrap:wrap;">
                            ...
                            <button class="btn btn-outline btn-sm" data-action="kategori-sec" data-kategori="${escapeHTML(kategori)}">Kullan</button>
                        </div>
                        <button class="btn btn-outline btn-sm" data-action="kategori-yenile" data-kategori="${escapeHTML(kategori)}">Begenmedim, Degistir</button>
                    </div>
    ...
    function analizModuGuncelle(data) {
        const isMcrt = isMcrtTest(mevcutTestTuru);
    ...
    function mcrtAnalizGorunumuKur(data) {
        ...
        if (!isMcrtTest(mevcutTestTuru) || secenekler.length === 0) {
    ...
    function aktifAnalizVerisiGetir() {
        ...
        let baz = [];
        if (isMcrtTest(mevcutTestTuru) && secim && sonAnalizPaket.blok_analizleri && sonAnalizPaket.blok_analizleri[secim]) {
    ...
    function analizGorunumuDegisti() {
        ...
        if (qDetails && isMcrtTest(mevcutTestTuru)) {
    ...
    function agGorunumuDegisti() {
        mevcutAgGorunumu = document.getElementById('networkViewFilter')?.value || 'ag';
        renderRadarChart(sonAnalizVerisi);
    }
    ...
    async function analizYukle() {
        ...
        try {
            ...
            if(data && data.ozet) { 
                ...
                const agSecici = document.getElementById('networkViewFilter');
                if (agSecici) {
                    agSecici.style.display = isMcrtTest(mevcutTestTuru) ? 'inline-flex' : 'none';
                    if (!isMcrtTest(mevcutTestTuru)) {
                        agSecici.value = 'ag';
                        mevcutAgGorunumu = 'ag';
                    }
                }
                ...
                if(data.kalite) {
                    ...
                    document.getElementById('qTrialPct').textContent = isMcrtTest(mevcutTestTuru) ? Math.round(data.kalite.ortalama_hiz || 0) : '%0';
                ...
                if(isMcrtTest(mevcutTestTuru) && qDetails) {
    ...
    async function renderKatilimciAnalizi() {
        ...
        const pager = document.getElementById('katilimciAnalizPager');
        ...
        try {
            ...
            const params = new URLSearchParams({
                page: String(katilimciAnalizSayfa),
                per_page: String(tabloSayfaBoyutu)
            });
            const res = await fetch(`/api/proje/${id}/katilimci_analiz?${params.toString()}`);
            ...
            const data = await res.json();
            const paging = data.sayfalama || { sayfa: 1, toplam_sayfa: 1, toplam_kayit: 0 };
            katilimciAnalizSayfa = Math.min(Number(paging.sayfa || 1), Number(paging.toplam_sayfa || 1));
            ...
            if(data.durum === 'basarili' && data.katilimcilar && data.katilimcilar.length > 0) {
                ...
                data.katilimcilar.forEach(k => {
                    ...
                    tr.innerHTML = `
                        ...
                        <td>
                            <button class="btn ${seciliTakipOturumId === k.oturum_id ? 'btn-success' : 'btn-outline'} btn-sm" data-action="katilimci-sec" data-oturum-id="${escapeHTML(k.oturum_id)}" data-pid="${escapeHTML(k.pid)}" data-isim="${escapeHTML(k.ad_soyad)}">${seciliTakipOturumId === k.oturum_id ? '✅ Seçildi' : 'Takip İçin Seç'}</button>
                        </td>
                    `;
                    tbody.appendChild(tr);
                });
                if (pager) pager.innerHTML = renderPagerHTML('katilimci', paging, 'Toplam katılımcı');
            } else {
    ...
    function radarAlanlariniHazirla(showHeatmap = false) {
        const canvas = document.getElementById('chartRadar');
        const heatmap = document.getElementById('chartRadarHeatmap');
        if (canvas) canvas.style.display = showHeatmap ? 'none' : 'block';
        if (heatmap) heatmap.style.display = showHeatmap ? 'block' : 'none';
        return { canvas, heatmap };
    }

    function radarMetricDegeri(row, tip = 'ag') {
        const isMcrt = mevcutTestTuru === 'mcrt';
        if (!row) return 0;
        if (!isMcrt) {
            return tip === 'bar' ? Number(row.explicit_pct || 0) : Number(row.implicit_skor || 0);
        }
        if (tip === 'ag' || tip === 'kategori') {
            return Number(row.mcrt_skor || row.implicit_skor || 0);
        }
        return Number(row.secilme_orani || row.explicit_pct || 0);
    }

    function isiHaritasiRenk(value) {
        const clamped = Math.max(0, Math.min(100, Number(value) || 0));
        const alpha = 0.12 + (clamped / 100) * 0.72;
        return { bg: `rgba(99,102,241,${alpha.toFixed(2)})`, low: clamped < 45 };
    }

    function renderHeatmapView(ozet) {
        const { heatmap } = radarAlanlariniHazirla(true);
        if (!heatmap) return;
        const labels = [...new Set(ozet.map(o => o.ifade))];
        const markalar = [...new Set(ozet.map(o => o.marka || o.cevap_metin))];
        if (labels.length === 0 || markalar.length === 0) {
            heatmap.innerHTML = '<div class="heatmap-empty">Bu gorunum icin veri yok.</div>';
            return;
        }
        const rows = labels.map(label => {
            const cells = markalar.map(marka => {
                const row = ozet.find(o => (o.marka === marka || o.cevap_metin === marka) && o.ifade === label);
                const value = radarMetricDegeri(row, 'isi');
                const renk = isiHaritasiRenk(value);
                return `<td class="heatmap-cell ${renk.low ? 'low' : ''}" style="background:${renk.bg};">${Math.round(value)}</td>`;
            }).join('');
            return `<tr><td class="heatmap-label">${escapeHTML(label)}</td>${cells}</tr>`;
        }).join('');
        heatmap.innerHTML = `<table class="heatmap-table"><thead><tr><th class="heatmap-label">Ifade</th>${markalar.map(m => `<th>${escapeHTML(m)}</th>`).join('')}</tr></thead><tbody>${rows}</tbody></table>`;
    }

    function renderKategoriChart(ozet) {
        const { canvas, heatmap } = radarAlanlariniHazirla(false);
        if (heatmap) heatmap.innerHTML = '';
        const kategoriOzet = kategoriBazliOzetUret(ozet).filter(r => (r.kategori || '').trim() !== '');
        chartBosMesaj('chartRadar', 'Kategori gorunumu icin kategori bilgisi yok.', kategoriOzet.length === 0);
        if (kategoriOzet.length === 0) {
            if (chartRadar) { chartRadar.destroy(); chartRadar = null; }
            return;
        }
        const labels = [...new Set(kategoriOzet.map(o => o.kategori))];
        const markalar = [...new Set(kategoriOzet.map(o => o.marka || o.cevap_metin))];
        if (chartRadar) chartRadar.destroy();
        chartRadar = new Chart(canvas, {
            type: 'bar',
            data: {
                labels,
                datasets: markalar.map((m, i) => ({
                    label: m,
                    data: labels.map(l => {
                        const d = kategoriOzet.find(o => (o.marka === m || o.cevap_metin === m) && o.kategori === l);
                        return radarMetricDegeri(d, 'kategori');
                    }),
    ...
    function renderBarDetayChart(ozet) {
        const { canvas, heatmap } = radarAlanlariniHazirla(false);
        if (heatmap) heatmap.innerHTML = '';
        if (!ozet || ozet.length === 0) {
            chartBosMesaj('chartRadar', 'Bu gorunum icin veri yok.', true);
            if (chartRadar) { chartRadar.destroy(); chartRadar = null; }
            return;
        }
        const labels = [...new Set(ozet.map(o => o.ifade))];
        ...
        const topLabels = labels.map(label => ({
            label,
            score: Math.max(...markalar.map(m => {
                const d = ozet.find(o => (o.marka === m || o.cevap_metin === m) && o.ifade === label);
                return radarMetricDegeri(d, 'bar');
            }))
        })).sort((a, b) => b.score - a.score).slice(0, 8).map(x => x.label);
        if (chartRadar) chartRadar.destroy();
        chartRadar = new Chart(canvas, {
            type: 'bar',
            data: {
                ...
                datasets: markalar.map((m, i) => ({
                    ...
                    data: topLabels.map(l => {
                        const d = ozet.find(o => (o.marka === m || o.cevap_metin === m) && o.ifade === l);
                        return radarMetricDegeri(d, 'bar');
                    }),
                    backgroundColor: colors[i % colors.length]
                }))
            ...
            options: {
                ...
                maintainAspectRatio: false,
                scales: {
                    x: { min: 0, max: 100, grid: { color: 'rgba(255,255,255,0.08)' } },
    ...
    function renderRadarChart(ozet) {
        const bos = !ozet || ozet.length === 0;
        if (chartRadar) { chartRadar.destroy(); chartRadar = null; }
        const secimEl = document.getElementById('networkViewFilter');
        if (secimEl) {
            secimEl.style.display = isMcrtTest(mevcutTestTuru) ? 'inline-flex' : 'none';
            if (!isMcrtTest(mevcutTestTuru)) {
                secimEl.value = 'ag';
                mevcutAgGorunumu = 'ag';
            } else {
                mevcutAgGorunumu = secimEl.value || mevcutAgGorunumu || 'ag';
            }
        }
        if (bos) {
            radarAlanlariniHazirla(false);
            chartBosMesaj('chartRadar', 'Bu filtrede veri yok.', true);
            const heatmap = document.getElementById('chartRadarHeatmap');
            if (heatmap) heatmap.innerHTML = '';
            return;
        }
        if (isMcrtTest(mevcutTestTuru) && mevcutAgGorunumu === 'isi') {
            chartBosMesaj('chartRadar', '', false);
            renderHeatmapView(ozet);
            return;
        }
        if (isMcrtTest(mevcutTestTuru) && mevcutAgGorunumu === 'kategori') {
            renderKategoriChart(ozet);
            return;
        }
        if (isMcrtTest(mevcutTestTuru) && mevcutAgGorunumu === 'bar') {
            renderBarDetayChart(ozet);
            return;
        }
        const { canvas, heatmap } = radarAlanlariniHazirla(false);
        if (heatmap) heatmap.innerHTML = '';
        chartBosMesaj('chartRadar', 'Bu filtrede veri yok.', false);
        const labels = [...new Set(ozet.map(o => o.ifade))];
        const markalar = [...new Set(ozet.map(o => o.marka || o.cevap_metin))];
        chartRadar = new Chart(canvas, { type: 'radar', data: { labels, datasets: markalar.map((m, i) => ({ label: m, data: labels.map(l => {
            const d = ozet.find(o => (o.marka === m || o.cevap_metin === m) && o.ifade === l);
            return radarMetricDegeri(d, 'ag');
        }), borderColor: colors[i % colors.length], backgroundColor: colors[i % colors.length] + '33', fill: true, pointRadius: 4 })) }, options: { responsive: true, maintainAspectRatio: false, scales: { r: { min: 0, max: 100, grid: { color: 'rgba(255,255,255,0.1)' }, angleLines: { color: 'rgba(255,255,255,0.1)' }, pointLabels: { color: '#94a3b8', font: { size: 10 } } } } } });
    }
    ...
    async function renderHamVeri() {
        const pager = document.getElementById('hamVeriPager');
        const params = new URLSearchParams({
            page: String(hamVeriSayfa),
            per_page: String(tabloSayfaBoyutu)
        });
        if (seciliTakipOturumId) params.set('oturum_id', seciliTakipOturumId);
        const res = await fetch(`/api/proje/${mevcutProjeId}/veriler?${params.toString()}`);
        const data = await res.json();
        ...
        if (filtreBilgi) {
            filtreBilgi.innerHTML = seciliTakipOturumId
                ? `Filtre: <strong>${escapeHTML(seciliTakipIsim)}</strong> <button class="btn btn-outline btn-sm" style="margin-left:.5rem;" data-action="katilimci-sec-kaldir">Temizle</button>`
                : 'Son 15 kayıt gösteriliyor';
        }
        const paging = data.sayfalama || { sayfa: 1, toplam_sayfa: 1, toplam_kayit: 0 };
        hamVeriSayfa = Math.min(Number(paging.sayfa || 1), Number(paging.toplam_sayfa || 1));
        ...
        tbody.innerHTML = data.veriler.map(v => `<tr><td>${escapeHTML(v.katilimci_id).substring(0,8)}</td><td>${escapeHTML(v.marka) || '-'}</td><td>${escapeHTML(v.ifade) || '-'}</td><td>${escapeHTML(v.cevap) || '-'}</td><td>${escapeHTML(v.sure_ms)}ms</td></tr>`).join('');
        if (pager) pager.innerHTML = renderPagerHTML('ham-veri', paging, seciliTakipOturumId ? 'Katilimci cevaplari' : 'Toplam ham kayit');
    }
    ...
    function temaDegistir() { document.body.classList.toggle('dark-mode'); }
    ...
    async function linkleriYukle() {
        if(!mevcutProjeId) return;
        const res = await fetch(`/api/proje/${mevcutProjeId}/linkler?_=${Date.now()}`);
        const data = await res.json();
                ...
                <tr>
                    ...
                    <td>
                        <div style="display:flex; align-items:center; gap:0.5rem; flex-wrap:wrap;">
                            ...
                            <button class="btn btn-outline btn-sm" style="padding:0.2rem 0.45rem; font-size:0.65rem;" data-action="link-yeniden-ac" data-token="${escapeHTML(l.token)}">Yeniden Ac</button>
                        </div>
                    </td>
                </tr>
            `).join('');
    ...
    async function linkYenidenAc(token) {
        ...
        if(data.durum === 'basarili') {
            toastGoster(data.mesaj);
            setTimeout(() => { linkleriYukle(); renderKatilimciAnalizi(); }, 500);
        } else {
            toastGoster('Hata: ' + data.mesaj);
        }
    }
    ...
    async function raporOlustur(includeAi = false) {
        ...
        const digerBtn = document.getElementById(includeAi ? 'btnRaporOlustur' : 'btnAiRaporOlustur');
        const eskiHtml = btn ? btn.innerHTML : '';
        if (btn) {
            btn.disabled = true;
            btn.classList.add('btn-loading');
            btn.innerHTML = '<span class="btn-loading-label">Hazirlaniyor...</span>';
        }
        if (digerBtn) digerBtn.disabled = true;
        toastGoster(includeAi ? "AI yorumlu rapor olusturuluyor, lutfen bekleyin..." : "Rapor olusturuluyor, lutfen bekleyin...");
        ...
        try {
            if (includeAi) {
                if (aiContainer && aiContainer.style.display !== 'none' && aiRaporProjeId === mevcutProjeId) {
                    aiText = aiContainer.innerHTML;
                } else {
                    const aiRes = await fetch(`/api/proje/${mevcutProjeId}/ai_rapor`, { 
                        method: 'POST',
                        headers: { 'X-CSRFToken': csrfToken }
                    });
                    ...
                    if (aiData.durum !== 'basarili') {
                        throw new Error(aiData.mesaj || 'AI yorumu olusturulamadi');
                    }
                    if (aiContainer) {
                        aiContainer.style.display = "block";
                        aiContainer.innerHTML = aiData.html;
                    }
                    aiRaporProjeId = mevcutProjeId;
                    aiText = aiData.html;
                    aiRaporSlidePack = aiData.slide_pack || null;
                }
            }
            ...
            const res = await fetch(`/api/proje/${mevcutProjeId}/analiz_rapor`, { 
                ...
                body: JSON.stringify({ ai_text: aiText, include_ai_text: includeAi, ai_slide_pack: includeAi ? aiRaporSlidePack : null })
            });
            ...
            if(data.durum === 'basarili') {
                toastGoster(includeAi ? "AI yorumlu rapor hazir! Paket indiriliyor..." : "Rapor hazir! Paket indiriliyor...");
                if (data.paket_url) {
                    const a = document.createElement('a');
                    a.href = data.paket_url;
                    a.download = data.paket_isim || 'rapor_paketi.zip';
            ...
            console.error(err);
            toastGoster("Rapor hatasi: " + err.message);
        } finally {
            if (btn) {
                btn.disabled = false;
                btn.classList.remove('btn-loading');
                btn.innerHTML = eskiHtml || (includeAi ? 'AI Raporu İndir' : 'Raporu İndir');
            }
            if (digerBtn) digerBtn.disabled = false;
        }
    }

    async function aiRaporluPaketOlustur() {
        return raporOlustur(true);
    }
    ...
    async function aiRaporUret() {
        ...
        try {
            ...
            if(data.durum === 'basarili') {
                ...
                aiRaporProjeId = mevcutProjeId;
                aiRaporSlidePack = data.slide_pack || null;
